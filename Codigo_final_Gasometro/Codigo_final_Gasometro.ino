...
const char name_device = 27;  ////device numero de grupo 5A 1x siendo x el numero de grupo
...
void fun_envio_mqtt ()
{
  ...
  if (flag_vacio == 0) ////si hay los envio
  {
    ...
    snprintf (mqtt_payload, 150, "Grupo:%u & Time:%ld & Temperatura:%.2f & MQ5:%.2f & MQ7:%.2f & P:%.2f & Luz:%.2f & Ruido:%.2f & Alarma:%u", name_device, aux2.time, aux2.T1, aux2.G5, aux2.G7, aux2.Presion, aux2.luz, aux2.ruido, aux2.Alarma); //random(10,50)
...
void fun_entra (void)
{
  ...
  datos_struct[indice_entra].T1 = T; /// leo los datos
...
{
  WiFi.mode(WIFI_AP);
  WiFi.softAP("WIFI GRUPO 7");///puede agregarle el nombre del grupo
...
Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT,
                         OLED_MOSI, OLED_CLK, OLED_DC, OLED_RESET, OLED_SC);
////////////////////////////////////////////////////////////////////
/////////////SETUP/////////////////////////////////////////////////
void setup() {
  Serial.begin(115200);
...
void loop() {
  if (contador == 2000) {
    ...
    checkBMP();
    Serial.println(checkMQ5());
    Serial.println(checkMQ7());
    Serial.println(checkLS());
    Serial.println(T, 2);
    Serial.println(P, 2);
    contador = 0;
  }
  if (checkMQ5() >= 50 || checkMQ7() >= 50) {
    playAlarm();
  } else {
    activeAlarm = false;
    auxAlarm = 0;
...
int checkMQ5() {
  ...
  int mapMQ5 = map(mq5, 0, 4095, 0, 100);
  return mapMQ5;
}
int checkMQ7() {
  float mq7 = analogRead(MQ7_PIN);
  int mapMQ7 = map(mq7, 0, 4095, 0, 100);
  return mapMQ7;
}
...
/*void displayData(int *values) {
  DisplayBMP180(values[checkBMP]);
  DisplayMQ7(values[checkMQ7]);
  DisplayMQ5(values[checkMQ5]);
  DisplayTEMT6000(values[checkLS]);
}*/
